...
def caja_y_bigotes_distribucion_concentraciones_CO_por_año(dataframe,anio):
    df=dataframe[(dataframe['Anio'] == anio) & (dataframe['Tiempo de exposición'] ==8) & (dataframe['Variable'] == "CO")]
    newdf=df.groupby(["Concentración"])
    #df.boxplot(column= "anio",by="Concentración")
    newdf.boxplot(column="Concentración")
    plt.show()
    #TODO terminar esta funcion 
caja_y_bigotes_distribucion_concentraciones_CO_por_año(x,2018)
#req5
